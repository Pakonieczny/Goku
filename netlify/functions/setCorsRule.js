  ...
  try {
    ...
    await storage.bucket("gokudatabase.firebasestorage.app")
      .setCorsConfiguration([{
        /*  Every origin that reads this bucket from a browser must be listed
            here. The Listing Generator moved to its own Netlify site, which
            was never added — so manifest.json (and any other direct fetch)
            came back as a CORS failure. Images survived only because they go
            through corsProxy server-side. */
        origin        : [
    ...
    return {
      statusCode: 200,
      body: "✅ CORS rule applied to: shipping-1.goldenspike.app, delicate-tanuki-616ac0.netlify.app, localhost:8888"
    };
  } catch (err) {
